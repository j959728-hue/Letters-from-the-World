...
def test_sync_preserves_existing_backup(tmp_path, monkeypatch):
    ...
    destination.mkdir()
    (destination / 'edition_1.epub').write_bytes(b'original')
    ...
    def fake_gh(*args):
        ...
        for name in ('edition_1', 'edition_2'):
            path = folder / name / 'briefing.epub'
    ...
    assert sync_names == ['edition_2.epub']
    assert (destination / 'edition_1.epub').read_bytes() == b'original'
    assert (destination / 'edition_2.epub').read_bytes() == b'new'
